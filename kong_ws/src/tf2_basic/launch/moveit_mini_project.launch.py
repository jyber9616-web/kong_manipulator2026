"""사용자 SRDF를 사용하는 OpenManipulator-X MoveItPy 미니 프로젝트 launch."""
...
def generate_launch_description() -> LaunchDescription:
    """공식 설정과 사용자 SRDF를 조합해 미니 프로젝트 노드를 실행한다."""
    ...
    tf2_basic_share = Path(get_package_share_directory("tf2_basic"))
    ...
    custom_srdf_file = (
        tf2_basic_share / "config" / "open_manipulator_x_custom.srdf"
    )
    ...
    moveit_py_parameters = {
        "robot_description_semantic": custom_srdf_file.read_text(encoding="utf-8"),
    ...
    mini_project_node = Node(
        package="tf2_basic",
        executable="moveit_mini_project",
        name="moveit_mini_project",
        output="screen",
        parameters=[
            moveit_py_parameters,
            robot_description,
